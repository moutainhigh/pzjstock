...
public class FilterStockRuleDateInit {
	...
	public List<AddStockModel> filterStockRuleDate(List<Long> ruleIds) {
		...
		Result<ArrayList<SkuStockListResult>> stockProResult = null;
		try {
			StockListParam stockListParam = new StockListParam();
			stockListParam.setStockIdList(ruleIds);
			stockProResult = skuStockService.findStockSkuByStockRuleIds(stockListParam);
		} catch (Exception e) {
			logger.error("filterStockRuleDate invoke findStockSkuByStockRuleIds error.", e);
		...
		if (stockProResult.isOk()) {
			...
			if (CommonUtils.checkObjectIsNull(stockProList)) {
				if (logger.isDebugEnabled()) {
					logger.debug("库存规则查询不到可用的产品列表---stockRuleIds:{},result:{}", ruleIds,
							JSONConverter.toJson(stockProResult));
				}
				return null;
			}
			...
			logger.error("通过库存规则ID查询产品列表异常，stockProResult:{}.", JSONConverter.toJson(stockProResult));
			throw new InvokeOtherServiceException();
		}
	}
}
