...
import com.pzj.core.stock.seat.TransferSeatChartDataEngine;
...
public class SeatChartServiceImpl implements SeatChartService {
	...
	@Resource(name = "transferSeatChartDataEngine")
	private TransferSeatChartDataEngine transferSeatChartDataEngine;
	...
	@Override
	public Result<Boolean> transferSeatChartData() {
		Result<Boolean> result = new Result<Boolean>(Boolean.FALSE);

		try {
			transferSeatChartDataEngine.transferSeatChart();
			result.setData(Boolean.TRUE);
		} catch (Throwable e) {
			logger.error("transferSeatChartData fail.", e);
			CommonUtils.convertException(e, result);
		}
